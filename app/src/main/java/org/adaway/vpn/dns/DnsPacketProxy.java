...
public class DnsPacketProxy {
    ...
    public void initialize(Context context) {
        AdBlockModel model = ((AdAwayApplication) context.getApplicationContext()).getAdBlockModel();
        Timber.d("DnsPacketProxy: initialize called. AdBlockModel: %s", model);
        if (model instanceof VpnModel) {
            this.vpnModel = (VpnModel) model;
            Timber.d("DnsPacketProxy: VpnModel assigned. Hash: %s", System.identityHashCode(this.vpnModel));
        } else {
            Timber.e("DnsPacketProxy: Model is NOT VpnModel! It is: %s", model != null ? model.getClass().getName() : "null");
            this.vpnModel = null;
        }
    }
    ...
    private HostEntry getHostEntry(String dnsQueryName) {
        ...
        Timber.d("DnsPacketProxy: getHostEntry called for '%s'", hostname);
        
        HostEntry entry = null;
        if (this.vpnModel != null) {
            Timber.d("DnsPacketProxy: vpnModel is valid (%s). Calling getEntry...", System.identityHashCode(this.vpnModel));
            entry = this.vpnModel.getEntry(hostname);
        } else {
            Timber.e("DnsPacketProxy: vpnModel is NULL in getHostEntry!");
        }
        
        if (entry == null) {
            Timber.d("DnsPacketProxy: Entry was null (or vpnModel null). Creating default ALLOWED entry.");
            entry = new HostEntry();
